...
var result = document.querySelector('.result'); // screen value

var equal = 0; //----------add to the Screen
...
document.querySelector('.numbers').addEventListener('click', function (event) {
  if (event.target.innerText.length == 1 && result.innerText == 0) {
    ...
    result.innerText = "".concat(result.innerText) + event.target.innerText;
  }
}); //----------------Clear
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "5706" + '/');
